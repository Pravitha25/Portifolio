...
// Helper: get responsive translateZ for slider
function getSliderRadius() {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let autoRotate;
  let currentRadius = getSliderRadius();

  function buildSlides(radius) {
    slider.innerHTML = "";
  ...
  slider.addEventListener("touchend", (e) => {
    ...
    if (Math.abs(diff) > 30) {
      rotateSlider(diff > 0 ? -1 : 1);
    }
    startAutoRotate();
  }, { passive: true });

  // Rebuild slides on resize with new radius
  let resizeTimer;
  window.addEventListener("resize", () => {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(() => {
      const newRadius = getSliderRadius();
      if (newRadius !== currentRadius) {
        currentRadius = newRadius;
        buildSlides(currentRadius);
        // Re-apply current rotation angle
        slider.style.transform = `rotateY(${angle}deg)`;
      }
    }, 200);
  });
